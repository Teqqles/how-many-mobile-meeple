...
class UrlFragmentEncoder {
  static String encode(String name, {required Items items, required Settings settings}) {
    var encodedItems = items.itemList.map((item) => item.name).join("+");
    var encodedSettings = settings.changedSettings.values.map((setting) => "${setting.name}=${setting.value}").join("&");
